...
object SearchPromptTools {
    ...
    fun buildSearchPlanThinking(queries: List<String>): String {
        if (queries.isEmpty()) return "正在判断是否需要联网检索，并准备获取可用信息。\n"
        return buildString {
            appendLine("正在拆解问题并并行检索多个信息方向。")
            appendLine("会优先覆盖时效性、可信来源和不同主题侧面。")
        }
    }

    fun buildSearchReadyThinking(results: List<TavilySearchResult>): String {
        if (results.isEmpty()) return "未检索到足够可靠的网页信息，准备基于已有上下文回答。\n"
        return buildString {
            appendLine("已获取 ${results.size} 条网页线索，正在去重和压缩来源信息。")
            appendLine("接下来会把相近内容合并为要点，并在正文中保留来源编号。")
        }
    }
