...
class ChatActivity : AppCompatActivity() {
    ...
    private fun ChatUiState.isStreamingResponse(): Boolean {
        if (!isLoading) return false
        val lastAssistant = messages.lastOrNull { it.role == Message.ROLE_ASSISTANT } ?: return false
        return lastAssistant.sources.isEmpty()
    }
